...
// TestSolarInject_WMaxLimPct_pct_RoundTrip is the RMD-046 API-level
// regression, solar's half of TestInject_WMaxLimPct_pct_RoundTrip in
// battery_test.go: the "WMaxLimPct_pct" Inject key must round-trip through
// the raw register and GET /state (Snapshot) at TRUE 0-100 percent units, not
// the old double-scaled convention (val*100 in, /100 out) that saturated the
// int16 register for any injected value above ~3.27. It also pins the
// explicit [0,100] clamp: WMaxLimPct is 0-100 per the DER model (SunSpec
// M123/M704), so an out-of-range percent is clamped rather than encoded into
// a representable-but-meaningless raw word.
func TestSolarInject_WMaxLimPct_pct_RoundTrip(t *testing.T) {
	const wmax = 8000.0
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			if err := ss.Inject([]byte(`{"WMaxLimPct_pct":` + jsonNum(tc.inject) + `}`)); err != nil {
				t.Fatalf("inject: %v", err)
			}
