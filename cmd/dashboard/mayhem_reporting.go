...
func cannotComplyTable27Scenario() *mayScenario {
	...
	return &mayScenario{
		...
		Expected:      "gridsim /admin/alerts records the CannotComply Response(s) for this control's mRID with vocab=table27 (IEEE 2030.5 Table 27 status codes — 4/5/8/10/252/253/254, SD-02: docs/design/SD02_RESPONSE_SEMANTICS_RC0_2026-08-17.md, lexa-gw), never vocab=legacy (the retired LEXA 0xF0 extension — Table 27 defines no manufacturer range at all).",
		HoldS:         75,
		Fix:           "internal/northbound/responses/tracker.go's SetLegacyCannotComplyCode wiring / cmd/northbound/config.go's LegacyCannotComplyCode default; verify the deployed northbound.json does not carry \"legacy_cannotcomply_code\": true. This scenario is NOT-APPLICABLE-FOR-RC0 pending the row-7 LogEvent/preference-producer fix (SD-02 adjudication #3), so this Fix pointer is for when it is re-enabled.",
		NotApplicable: "F2/SD-02 adjudication #3 (docs/design/SD02_RESPONSE_SEMANTICS_RC0_2026-08-17.md, lexa-gw): this scenario's setup forces a fault-class breach (ack_before_effect — the inverter ACKs curtailment writes but never applies them, with the battery held full so PV curtailment is the only, blocked, lever). Its oracle demands gridsim eventually record a CannotComply Response for this control's mRID (either vocab), then grades which status-code vocabulary it used. Per SD-02, the ONLY live producer of the 4/5/8/10 status arc (standard-mode CannotComply) is the preference-class bus.ComplianceAlert publisher on bus.TopicCSIPComplianceAlert, and it is INERT in this product — it was the abandoned lexa-hub's orchestrator's topic (lexa-hub-abandoned.md, 2026-08-03). A correct RC0 gateway posts NO onset/end-of-event status for a fault-caused breach like this one (SD-02 rows 6/11) — not even the legacy 0xF0 form, which shares AlertCannotComply's cause gating. So the base convergence oracle this diagnoser layers on (diagnoseConverge) always finds a sustained, unexcused breach and FAILs, and the vocabulary check never gets a real alert to classify. Real fix: a gateway-originated LogEvent producer for control faults (SD-02 rows 6/11's alarm half) is a registry item, not implemented this wave; once it lands, re-point this scenario at journal/LogEvent evidence for the fault-admission half, and keep this file's vocabulary check only for whatever genuinely reaches AlertCannotComply.",
		setup: func(d *mayhemDriver) (*activeConstraint, error) {
