...
| P1 Shared modules (R2) | 019–024, 082 | IN PROGRESS (2026-07-05) | — | TASK-019 DONE (skeleton + go.work + AD-003 mechanics). **TASK-020 DONE (partial):** `sunspec`+`modbus` codecs extracted to `lexa-proto` byte-identical (product authority per AD-003), lexa-hub consumes via go.work, in-repo copies deleted; disposition inventory `docs/refactor/notes/TASK-020-sunspec-disposition.md` — legacy maps (103/121/123/802/201-203) were byte-identical across forks, all 701-714 divergences → product, **0 port-from-bench** (the one bench fix `e1d7ba2` already in product), 0 investigate. **derbase deferred to TASK-023** (unanticipated `device`→`model` cross-internal coupling). Bench flip+conformance+campaign batched to TASK-021. |
