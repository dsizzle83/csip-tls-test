...
| P3 Time & persistence | 034–043 | IN PROGRESS | — | 034 utilitytime core + 035 walker/scheduler (guards verbatim, failclosed untouched) + 036 hub/api/optimizer + 037 local clock-step (monotonic anchoring) all DONE + MERGED — **W4 closed, one time owner**. 039 journal library DONE + MERGED (zero consumers). 038 (local-clock-step Mayhem scenarios `local-clock-step-forward`/`-back`, GAP-04, validates 037's monotonic anchoring) CODE COMPLETE — `task/038-clock-step`; 10×-solo stability runs + campaign inclusion batched for a later bench session (soak owns the bench). Remaining: 040/041 (journal + snapshot wiring, need 031's breach component — now merged), 042/043 (retained-trust + power-cut scenarios). |
