...
| P6 Commercialization | 068–081 | IN PROGRESS | — | DONE + MERGED: 068 (northbound 1206→241, 4 pkgs), 072 (cert expiry monitoring + /status), 080 (curve de-scope AD-010). 046 (async publishes, §11) merged — radioactive, mqtt-scenario gate at 081. DONE (branch, unmerged): 069 — AD-009 resolved to option (b): kept the fuzz-clean/capped httpwire parser and ADDED chunked-transfer decoding to it (closes the utility-interop gap; net.Conn shim deferred as P6-with-time backlog). httpwire-local, CGo-free, re-fuzzed 0 crashers; no transport/timeout/wire-byte change so no conformance dual-run needed for (b). **073 PARTIAL (2026-07-06, `lexa-hub` `task/073-cert-rotation` @ `90d5d9a`, unmerged):** cert-rotation mechanism code-complete — `WolfSSLFetcher.Reload` probe-then-commit swap (dial+probe the new session fully before touching the old one; Close→FreeSSL→FreeCtx ordering) + `RotationController` sentinel-file trigger (rotates discovery/response/flow-reservation one at a time, LFDI-mismatch refusal) + operator script/runbook; unit + real-wolfSSL integration tests green (`go test -race ./internal/... ./cmd/...` and `-tags=integration` both green). Bench acceptance (live single-rotation meter-continuity drill, and — the task's other headline deliverable — the 24h reconnect-churn soak) explicitly NOT done this session, soak-gated like P5's residuals; runbook + `scripts/cert-churn-soak.sh` ready in csip-tls-test. RSK-07 updated to "mitigation implemented, soak pending". **076 CODE COMPLETE (2026-07-06, `csip-tls-test` `task/076-scenarios-as-data`, unmerged):** Mayhem scenarios-as-data — JSON spec schema + interpreter (`cmd/dashboard/scenariospec.go`), full v1 action vocabulary mirroring `mayhemDriver`'s methods 1:1, oracle registry (oracles stay Go, 8 seeded incl. parameterized `diagnoseSurvival`), `-scenario-dir` re-read per run (no rebuild/restart to add a scenario — kills the D8 stale-binary trap), ID-collision-is-a-logged-load-error-never-a-silent-shadow. Pilot (`qa/scenarios/export-cap-full-battery.json`) + two more twins (`grid-disconnect`, `wan-outage-hold`, proven inline in `scenariospec_test.go`) unit-parity-proven against fake bench backends (call-for-call match with their Go literals); `go test -race ./cmd/dashboard/...`, `make qa`, `make test-fast`, `go test ./tests/` green, `bin/dashboard` builds clean. Bench acceptance (live ×3 parity with the Go twin disabled, live no-rebuild demo, one full FAST campaign proving the loader is a no-op for the Go set) deferred to 081/next deploying session — same pattern as 041/042/043/073. 077 (mass scenario migration) unblocked to start once 076 merges. Remaining implementable: 070/071 (ctx, poll-rate), 074 (OCPP SP2 enablement), 077 (scenario migration, needs 076 merged first). External-gated (081 pending-markers): 075 golden fixtures (vendor HW), 078 30-day soak, field pilot, cert lab. Then 081 = single comprehensive bench validation of all accumulated code-complete-bench-pending work + checklist. |
