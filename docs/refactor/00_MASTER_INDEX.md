...
|---|---|---|---|---|
| P0 Foundations | 001–018 | IN PROGRESS | — | Done: 001, 002, 003, 004, 005, 010, 011, 012, 014, 016, 017 (2026-07-04; details in task headers). 012: dead SetCSIPPrograms/e.sched engine dual-source path removed (lexa-hub 8844f68, branch task/012-dead-engine-path, awaiting review/merge); full FAST campaign 33P/17D/1F/0B, sole FAIL = known-flaky control-churn (qa-mayhem-20260704-211243.md). Partial: 007/008/009 (code merged; bench deploy + wedge test + soak + journald measurement at P0 exit); 013 (code+tests done on `task/013-broker-acls` both repos, lockstep, not yet merged — `mqttutil.ConnectAuth`/per-service creds, `mosquitto-lexa.acl` matrix, `deploy-hub-pi.sh --enable-mqtt-acl` staged flip, `mqttproxy` credentialed CONNECT; bench rollout + targeted mqtt-* ×10 + full FAST campaign RSK-09 gate deferred to P0 exit per deploy freeze ahead of 012 merge — see TASK-013 header); 014 (code merged; token flip + QA gate at P0 exit); 015 (`scripts/mayhem-campaign.sh` + `docs/QA_STOCK_TRIAGE_TEMPLATE.md` built + locally tested 2026-07-04; the actual 10-cycle STOCK baseline + triage + `QA_REPORT_STOCK_M0` report deferred to the P0-exit gate, tooling-only wave per launch instructions — see TASK-015 header). Open: branch protection + `LEXA_HUB_RO_TOKEN` PAT secret (human, AD-012/TASK-004); 006 worklist = 2 reachable vulns (VULN_BASELINE_2026-07-04.md). Next: STOCK baseline (015 completion), 018, 006, P0-exit gate (007/008/009/013/014/015 bench+campaign rollup). |
