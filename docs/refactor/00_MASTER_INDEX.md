...
| P6 Commercialization | 068–081 | IN PROGRESS | — | 068 DONE + MERGED (northbound god-file 1206→241 lines, 4 packages, pure move, cgo amd64+arm64; 4 fail-closed scenarios batched to next bench session). 080 DONE + MERGED (AD-010: curve functions DE-SCOPED for V1.0 with drafted conformance-statement language; 10-hop impl gap + backlog). **072 code+unit-tests DONE (2026-07-06, `lexa-hub` `task/072-cert-expiry` @ `26adab9`, unmerged):** cert-expiry monitor (`cmd/northbound/certmon.go`, startup+24h ticker), retained `lexa/northbound/certstatus` (`bus.CertStatus`), lexa-api `/status` `cert_status` field, WARN≤30d/ERROR-on-expiry alarm; `go test -race ./internal/... ./cmd/...` green. Bench validation (1-day short-lived cert, live WARN/ERROR + recovery) not done this session — remains before acceptance criteria close. Remaining: 069 (HTTP client ADR), 070–071 (ctx, poll-rate), 073 (cert rotation — consumes 072's monitor), 074 (OCPP SP2 enablement), 075 (golden fixtures — hardware-gated), 076/077 (scenarios-as-data), 078 (30-day soak — clock-gated), 081 (release gate). |
