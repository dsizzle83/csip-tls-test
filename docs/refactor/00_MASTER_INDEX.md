...
| P6 Commercialization | 068–081 | IN PROGRESS | — | DONE + MERGED: 068 (northbound 1206→241, 4 pkgs), 072 (cert expiry monitoring + /status), 080 (curve de-scope AD-010). 046 (async publishes, §11) merged — radioactive, mqtt-scenario gate at 081. DONE (lexa-hub branch task/070-ctx-propagation, unmerged): 070 — ctx propagation walker-deep: discovery.Fetcher.Get + Walker.Discover + all fetchX helpers are ctx-first, funneled through fetchAndParse's single ctx.Err() choke point; run.Discovery.RunOnce(ctx) classifies a shutdown-cancel walk (errors.Is context.Canceled/DeadlineExceeded) as Info-logged, not a fail-closed WARN/failure-counter increment (fail-closed hold-last-good unchanged either way — no publish on any walk error); lexa-telemetry gets a parallel WolfSSLFetcher.PostContext (Post itself untouched — responses.Tracker/flowres.Manager stay on it, out of blast radius) with its POST loop signal-bridged to ctx. New cancel-mid-walk unit test (stalling fake fetcher) returns in ~20ms; go test -race ./internal/... and ./cmd/... green; amd64+arm64 cgo builds green. Bench stop-during-walk timing + Mayhem smoke (wan-outage-hold, northbound-hang) pending a live bench session. Remaining implementable: 069 (HTTP client ADR), 071 (poll-rate), 073 (cert rotation), 074 (OCPP SP2 enablement), 076/077 (scenarios-as-data). External-gated (081 pending-markers): 075 golden fixtures (vendor HW), 078 30-day soak, field pilot, cert lab. Then 081 = single comprehensive bench validation of all accumulated code-complete-bench-pending work + checklist. |
