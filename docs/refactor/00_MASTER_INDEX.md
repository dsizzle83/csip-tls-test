...
| P5 Optimizer split (R4) | 056–067 | IN PROGRESS | — | Critical path. **Framework DONE + PROVEN + MERGED:** 056 behavioral tests, 057 plant model, 058 constraint skeleton (bounds-intersection arbitration), 059 shadow harness, **060 export constraint — SHADOW-VALIDATED: 0 divergence across the export scenario family + full campaign (34P/17D/0F/0B, zero control impact); mutation-verified; M2 boundary-flake designed out via adaptive detection window.** R4 approach proven end-to-end. Export axis behaviorally flip-ready (held in shadow pending the ≥1-week soak — see deadline note). **061 import+gen constraints SHADOW-VALIDATED** (2026-07-06, lexa-hub `task/061-import-gen`): ImportLimitConstraint + GenLimitConstraint (both TierCompliance) ported field-for-field, meter-independent gen floor + import NaN-hold preserved verbatim and mutation-verified, all three compliance constraints now in the candidate Stack in SHADOW. Gen + import scenario families 0 divergence; full FAST campaign 33P/18D/**0F/0B** with exactly 1 divergence — export-axis (battery charge at low SOC), NOT import/gen. Import+gen axes flip-ready pending soak; flips not executed. REMAINING (same proven pattern): 062 session consolidation, 063 economic isolation, 064 constants→plant, 065 multi-device, 066 delete cascade + the shadow→active flips. |
