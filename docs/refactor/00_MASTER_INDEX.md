...
| P1 Shared modules (R2) | 019–024, 082 | IN PROGRESS (2026-07-05) | — | 019 DONE (skeleton + go.work + AD-003 mechanics). 020 DONE (sunspec+modbus → lexa-proto, product authority, 0 port-from-bench, RSK-02 zero live exposure; derbase deferred to 023; disposition doc in `docs/refactor/notes/`). 022 DONE (ocppserver → lexa-proto, both consumers flipped, old copies deleted). **AD-003(e) Principal decision:** hosted-CI resolution gap (GOWORK=off can't see ../lexa-proto) closed by interim **vendoring** — `require lexa-proto v0.0.0` + `replace` + committed `vendor/` in both consumers; vendor mode never resolves the replace target so single-repo CI builds green; go.work still wins locally; removed at TASK-024 hosting. **023 DONE (partial)** (csipmodel → lexa-proto: product-authoritative union of both model forks, zero unresolved conflicts — bench fork was a strict subset with identical XML tags; both repos flipped + re-vendored, both model packages deleted; scope addendum — derbase also moved to lexa-proto, `internal/southbound/device.Measurements` now a type alias to `derbase.Measurements` (option (a), zero call-site changes); disposition doc `docs/refactor/notes/TASK-023-model-disposition.md`; bench-dependent steps 6-8 (live gridsim↔hub walk, targeted Mayhem, conformance regen) deferred — no bench deploy this session, batched to next deploying session). **021 DONE** (sims + conformance runner/tests + bench `derbase`/`battery`/`inverter` repointed to `lexa-proto/sunspec`+`lexa-proto/modbus`; bench fork deleted; legacy-model paths byte-identical (mechanical only), 701-712 read paths adapted to the shared layout engine, untested/uncalled M702+705-712 read/write surface deleted rather than re-implemented against a structurally different protocol — TASK-082 disposes the rest; `scripts/ci/lockstep-check.sh` patched to treat both-repos-retired as in-lockstep, not an error, gate now passes 0 divergences; also closed TASK-023's deferred bench-dependent steps 6-8 in this same deploying session — live gridsim↔hub discovery walk verified via journal evidence, targeted register-codec Mayhem set + full FAST campaign both run). Same-session hub+sim deploy (auth+ACL on); full FAST campaign 34P/17D/0F/0BLIND (51/51, within 32-35P/16-19D/0F band); conformance ×3 all PASS (inverter 19/19, battery 22/22, meter 9/9); CSIP conformance layers 1-3 (`run-conformance.sh`) 3/3 PASS. Next: 024, 082 (bench derbase/model-fork disposal, csip discovery/scheduler fork AD). |
