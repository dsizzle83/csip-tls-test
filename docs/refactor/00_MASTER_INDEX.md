...
| P4 Observability & QA depth | 044–055 | IN PROGRESS (2026-07-05) | — | Parallel track. 047 DONE (code+CI on lexa-hub `task/047-httpwire`, review/merge pending): tlsclient parsing core → CGo-free `internal/tlsclient/httpwire` leaf; missing 64 KiB header cap added; 3 fuzz targets seeded with 11 real gridsim responses, 15 min/target clean (0 crashers); nightly CI fuzz job (no sysroot needed). AD-009 updated — corpus + zero-findings feed the TASK-069 shim-vs-harden decision. Bench items (conformance smoke, Mayhem spot runs) batched to next deploying session. 045 DONE (code+tests on lexa-hub `task/045-logging`, review/merge pending): all six services on slog (svc= attr, log_level config, transition-not-steady-state — 3 per-tick lines demoted Info→Debug); lexa-api's plan heartbeat now ACTS on retained lexa/hub/plan (never/ok/stalled, arrival-time based, edge-triggered alarm + 2 gauges, additive /status field); AD-011 documented for operators (lexa-hub/docs/OPERATIONS.md). Bench validation (stop/start proof, Mayhem spot runs, full campaign) batched to next deploying session. |
