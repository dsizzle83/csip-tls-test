...
|---|---|---|---|---|
| P0 Foundations | 001–018 | IN PROGRESS | — | TASK-001 DONE 2026-07-04 (both repos clean/pushed, main==lexa-hub in csip-tls-test, AD-012 hosting decision recorded); branch protection on `main` NOT yet enabled in either GitHub repo — `gh`/API auth unavailable in the execution environment, see AD-012 for the exact command a human needs to run. TASK-016 DONE 2026-07-04 (lexa-hub@ac673ac, branch `task/016-qos-policy`, not yet merged): `bus.PubQoS` closes D5 (measurement-plane QoS 0, control-plane QoS 1, previously hardcoded QoS 1 everywhere); targeted Mayhem scenarios batched into the post-merge wave-gate campaign per Principal's instruction, not yet run. Next: TASK-002/003 (CI, unblocked). |
