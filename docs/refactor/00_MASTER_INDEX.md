...
| P2 Device Reconciler (R1) | 025–033 | IN PROGRESS (2026-07-05) | — | Critical path. 025 DONE: AD-013 desired-state schema + bus types (`internal/bus/desired.go`, unused) + `PRESERVATION_LEDGER.md` (11 rows, every legacy convergence mechanism → gate scenarios; 8/11 line-cites re-verified/corrected). AD-002 open questions closed (meter: no desired doc; interlock: measurement-only). 026 DONE (lexa-hub aba9988, unwired per 05 §12): pure I/O-free reconciler core `internal/reconcile` — write-on-diff / verify-by-readback / reassert-on-reconnect / retry-backoff / non-convergence + AD-013 seq/staleness/NaN/SeqReset; subsumes ledger L1–L4 semantics; injected clock; exhaustive table-driven suite `go test -race`, 99.4% coverage; ledger rows unchanged (replaces nothing yet). 027 CODE COMPLETE (lexa-hub 3d52412, unpushed branch `task/027-battery-shadow` both repos): hub-side battery desired-doc publisher (`cmd/hub/desired.go`, additive, wraps legacy actuator) + lexa-modbus shadow reconciler (`cmd/modbus/reconcile_shadow.go`, recorder only, zero writes); `bus.SupportedV` gets the deferred `lexa/desired/` case; `modbus.json` `"reconciler"` config gate (`off`/`shadow`/`active`, `active` rejected at load). Found + fixed in the same commit: `internal/reconcile.matches()` had an order-dependent completeness bug (map-iteration-dependent) hit by the first real multi-field (SetpointW+Connect) consumer — now deterministic, regression test added. Bench soak, verdict/campaign evidence, and PRESERVATION_LEDGER.md status flip (`legacy-active`→`shadow`) are the wave-gate follow-up (session scope: code+tests only, `go test -race ./internal/... ./cmd/...` green, no bench deploy). |
| P3 Time & persistence | 034–043 | IN PROGRESS (2026-07-05) | — | 039 DONE (lexa-hub, branch `task/039-journal`): `internal/journal` — append-only NDJSON event journal library (AD-005): schema v1 (control_adopted/control_released/dispatch/breach_begin/breach_end/cannot_comply_posted/service_start/snapshot_written/snapshot_restored, transition-only per 05 §9), size rotation (rename-then-create shift chain), batched fsync (FlushEvery/FlushInterval, lazy-checked on Append, no library goroutines), crash-tolerant reader (`Scan`, tolerates a torn tail line), optional `internal/metrics` hook. Write budget documented with real arithmetic in the package doc comment, referencing `docs/FLASH_BUDGET.md`'s measured 108 lines/min hub rate; default footprint (5 MiB) fits inside the existing 200 MB journald budget, not stacked on top. `go test -race ./internal/journal/` green, 95.1% coverage. Zero consumers (pure library; TASK-040 wires it, blocked on TASK-031). |
| P4 Observability & QA depth | 044–055 | IN PROGRESS (2026-07-05) | — | Parallel track. Done + merged: 044 (metrics ×6 services), 045 (slog + plan heartbeat, arrival-time based), 047 (httpwire leaf + 64 KiB header cap + nightly fuzz, 0 crashers), 048 (5 XML/bus fuzz targets, shared seed corpus, namespace-lore corrected + tripwire; findings: Time.CurrentTime ungated, csipref scheduler gate-less — backlogged; csipref CI gap closed). All bench validation items batched to the P2 wave-gate deploying session. |
