...
| P3 Time & persistence | 034–043 | **CODE COMPLETE — bench validation pending (2026-07-06)** | — | 034 utilitytime core + 035 walker/scheduler (guards verbatim, failclosed untouched) + 036 hub/api/optimizer + 037 local clock-step (monotonic anchoring) all DONE + MERGED — **W4 closed, one time owner**. 039 journal library DONE + MERGED (zero consumers). 040 journal wiring code complete (`lexa-hub` merge `38496e0`). **041 (2026-07-06, `lexa-hub` `task/041-snapshot`, merged to main @7af1ff3, hub-side only):** hub-side breach-episode snapshot (atomic tmp+rename, 60 s while-active resave, restore-on-start behind `hub.json` `snapshot.enabled` default off) code complete + unit-tested (`go test -race ./internal/... ./cmd/...` green); northbound-side `responseTracker` persistence and all bench acceptance criteria (live restart evidence, `hub-restart-mid-cap` 10×, flag-on/flag-off campaigns) not done this session — see AD-005's TASK-041 update. **042 (2026-07-06, `lexa-hub` `task/042-retained-trust` @ `dd62fe8`, merged to main @a61da0d):** retained-control staleness bound (adoption-time only, enforce-but-verify, never fail-open) + corrupt-retained re-request mechanism (`bus.TopicCSIPRewalk`, `mqttutil.SubscribeDecodeErr`, northbound `lastPublishedStore`/rewalk handler) code complete + unit-tested (`go test -race ./internal/... ./cmd/...` green); AD-006 extended. Bench acceptance (live truncated-payload injection, gate scenarios 10×, full campaign) explicitly out of scope this session — deferred to TASK-043. **043 CODE COMPLETE (2026-07-06, `csip-tls-test` `task/043-powercut`, merged to main @87882d2):** `power-cut-retained-rollback` (GAP-01, unclean SIGKILL+store-rollback) and `corrupted-retained-control` (GAP-02, truncated payload + WAN-dark + hub restart) scenarios added to `cmd/dashboard/mqtt_scenarios.go`, validating 042's staleness bound + rewalk re-request path; custom ladder + broker-store/parse helpers unit-tested (`go test ./cmd/dashboard/...` green); bench validation (10× solo each, abort-safety, full campaign) deferred to the 081 gate per this task's launch instructions — see `docs/QA_FINDINGS.md` §8. **038 (2026-07-06, `csip-tls-test` `task/038-clock-step`, merged to main @1436c47):** local (hub Pi) clock-step scenario code complete + unit-tested; bench validation (10× solo, abort/self-heal proof, campaign inclusion) deferred to the next batched wave gate. Remaining (bench validation only — no code outstanding except 041's NB half): 038's 10×-solo + campaign inclusion, 041's northbound half (not yet built) + hub-side bench validation, 042's bench acceptance criteria, 043's bench validation. |
