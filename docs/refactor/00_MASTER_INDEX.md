...
| P6 Commercialization | 068–081 | IN PROGRESS | — | DONE + MERGED: 068 (northbound 1206→241, 4 pkgs), 072 (cert expiry monitoring + /status), 080 (curve de-scope AD-010). 046 (async publishes, §11) merged — radioactive, mqtt-scenario gate at 081. **074 PARTIAL (2026-07-06, `task/074-ocpp-sp2` both repos, unmerged):** OCPP SP2 confirmed already-implemented both sides (`lexa-proto/ocppserver` TLS+constant-time BasicAuth, evsim `-tls-ca`/`-auth-user`/`-auth-pass`) — this task was enablement/provisioning. CSMS cert issued (IP SAN 69.0.0.1, bench CA), `deploy-hub-pi.sh --enable-ocpp-sp2` (cert staging + idempotent auth-secret gen + ocpp.json patch), `update-sim-pis.sh --enable-ocpp-sp2` (wss/-tls-ca/-auth-user/-auth-pass, idempotent both directions incl. rollback), negative-auth unit test (`cmd/ocpp` `TestOCPPSecurityProfile2_BasicAuth`, lexa-hub `go test -race ./internal/...` + this test green), product-config policy documented (ws:// bench-only, wss:// default). Bench lockstep deploy + 7-scenario ×3 EV Mayhem re-run deferred to 081 (runbook: csip-tls-test docs/BENCH.md "OCPP Security Profile 2" section). Remaining implementable: 069 (HTTP client ADR), 070/071 (ctx, poll-rate), 073 (cert rotation), 076/077 (scenarios-as-data). External-gated (081 pending-markers): 075 golden fixtures (vendor HW), 078 30-day soak, field pilot, cert lab. Then 081 = single comprehensive bench validation of all accumulated code-complete-bench-pending work + checklist (incl. 074's bench half). |
