...
| P4 Observability & QA depth | 044–055 | IN PROGRESS | — | Parallel track. DONE + MERGED: 044 metrics, 045 slog+heartbeat, 047 httpwire+fuzz, 048 XML/bus fuzz, 049/050/051 (dup-client-ID, disk-full, mqtt-storm), 052 (netem harness), 053 int16/scale sweep (65M execs 0 crashers), 054 (dither sweeps, extended-set), 055 NaN-string bus robustness — all code-merged, bench validation batched to next deploying session. **046 CODE COMPLETE (2026-07-06, `lexa-hub` `task/046-async-publish` @ `1418cc6`, unmerged):** async actuator publishes (`mqttutil.PublishJSON(Retained)Async` + `PendingPub.Harvest`) against `cmd/hub/desired.go`'s post-TASK-032 desired-doc actuators (this repo's task file predates TASK-032 and named the now-deleted `cmd/hub/actuators.go`/`cmdDeduper` — implemented 1:1 in spirit against the current actuators instead); harvest-next-call + one free opportunistic post-fire check, dedupe-baseline rollback on failure/timeout mutation-tested by hand; plan log goes async too, compliance alert stays sync bounded at 1s (`mqttutil.PublishJSONTimeout`); `lexa_hub_tick_overruns_total` now has a real source (50%-of-engine-interval budget, `tickTiming` accumulator). `go test -race ./internal/... ./cmd/...` green. Bench gates (mqtt-broker-latency/-restart 10×, tick-overrun metric curl, full FAST campaign) deferred to next bench session — radioactive-zone discipline (one-per-PR, full campaign, not same-day merge) still applies before merge. |
| P5 Optimizer split (R4) | 056–067 | IN PROGRESS | — | Critical path. **Framework DONE + PROVEN + MERGED:** 056 behavioral tests, 057 plant model, 058 constraint skeleton (bounds-intersection arbitration), 059 shadow harness, **060 export constraint — SHADOW-VALIDATED: 0 divergence across the export scenario family + full campaign (34P/17D/0F/0B, zero control impact); mutation-verified; M2 boundary-flake designed out via adaptive detection window.** R4 approach proven end-to-end. Export axis behaviorally flip-ready (held in shadow pending the ≥1-week soak — see deadline note). REMAINING (same proven pattern): 061 import+gen constraints, 062 session consolidation, 063 economic isolation, 064 constants→plant, 065 multi-device, 066 delete cascade + the shadow→active flips. |
| P6 Commercialization | 068–081 | IN PROGRESS | — | 068 DONE + MERGED (northbound god-file 1206→241 lines, 4 packages, pure move, cgo amd64+arm64; 4 fail-closed scenarios batched to next bench session). 080 DONE + MERGED (AD-010: curve functions DE-SCOPED for V1.0 with drafted conformance-statement language; 10-hop impl gap + backlog). Remaining: 069 (HTTP client ADR), 070–074 (ctx, poll-rate, cert lifecycle, OCPP SP2 enablement), 075 (golden fixtures — hardware-gated), 076/077 (scenarios-as-data), 078 (30-day soak — clock-gated), 081 (release gate). |
