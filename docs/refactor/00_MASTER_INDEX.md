...
| P1 Shared modules (R2) | 019–024, 082 | IN PROGRESS (2026-07-05) | — | TASK-019 DONE: `~/projects/lexa-proto` skeleton (5 empty packages) + `go.work` committed in both consumer repos + CI `GOWORK=off` guard; AD-003 extended with module path (bare `lexa-proto`, flips to hosted path when `dsizzle83/lexa-proto` exists), pinning mechanism (`proto.pin` SHA file until hosting+credential land), package layout, go.work policy. No code moved yet — unblocks TASK-020/022/023. TASK-022 BLOCKED-PARTIAL (2026-07-05, branch `task/022-ocppserver` in all three repos): `ocppserver` extracted to lexa-proto and both consumers' imports flipped, all named regression tests green under `go.work` — but this surfaced a real gap AD-003(d) didn't cover: once a consumer actually imports a `lexa-proto` package, hosted CI's `GOWORK=off` jobs (lexa-hub `vet-build-test`/`cross`; csip-tls-test `cgo-fast`) fail outright (`package lexa-proto/ocppserver is not in std`) because no replace directive exists and no job checks out a sibling `lexa-proto`. Old `internal/ocppserver/` copies deliberately left in place (not deleted) in both repos as a rollback safety net pending a Principal decision (CI job-level checkout, scoped replace exception, or accept red CI until TASK-024's pin gate + hosting land). Not merged. |
