...
| P6 Commercialization | 068–081 | IN PROGRESS | — | DONE + MERGED: 068 (northbound 1206→241, 4 pkgs), 072 (cert expiry monitoring + /status), 080 (curve de-scope AD-010). 046 (async publishes, §11) merged — radioactive, mqtt-scenario gate at 081. Remaining implementable: 069 (HTTP client ADR), 070/071 (ctx, poll-rate), 073 (cert rotation), 074 (OCPP SP2 enablement), 076/077 (scenarios-as-data). External-gated (081 pending-markers): 075 golden fixtures (vendor HW), 078 30-day soak, field pilot, cert lab. Then 081 = single comprehensive bench validation of all accumulated code-complete-bench-pending work + checklist. |
