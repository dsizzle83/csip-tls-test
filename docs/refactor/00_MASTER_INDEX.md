...
| P6 Commercialization | 068–081 | IN PROGRESS | — | Order hardware during P2. **068 DONE out-of-order (2026-07-06, lexa-hub `task/068-northbound-decomp` @ `d2f135c`, unmerged)** — cmd/northbound/main.go god-file (1206 lines, grown past the task's stale 831 estimate by TASK-042's merge) decomposed into internal/northbound/{flowres,responses,publish,run} by 4 bisectable pure-move commits + a docs commit; main.go now 241 lines wiring-only (over the ≤150 target — irreducible fetcher/journal/metrics/subscription wiring, flagged for review). `make test` + `go vet` + amd64/arm64 cgo builds green; csip-tls-test `go test ./tests/` green. One deviation (dropped a provably-unused `cfg *Config` param to avoid an import cycle) documented in the commit message and task file. Bench smoke (journal-line diff + 4-scenario Mayhem set) and `hub-replay-tune.sh fast` NOT run — no bench access this session; next bench session must complete before merge. **079 DONE out-of-order (2026-07-06, lexa-hub `task/079-dst-tou`, unmerged)** — ran early per its own scheduling note (gated only on 036, honoring P1 GAP-05 priority): DST/leap-smear TOU boundary tests + one inline `OptimalChargeWindow` fix (spring-forward gap hour-collapse undercounted window cost; no production caller today, zero bench impact); SOM-zone-matches-tariff-zone deployment requirement documented in lexa-hub CLAUDE.md; one KNOWN-GAP backlogged (25-hour fall-back day repeated-hour window-start limitation). |
