...
| P5 Optimizer split (R4) | 056–067 | IN PROGRESS | — | Critical path. 056 DONE (unblocker): orchestrator decision-string oracles (`hasDecisionContaining`, `contains(d.Reason,"cooldown")`) replaced with behavioral plan assertions (solar `CurtailToW`, EVSE `MaxCurrentA`), mutation-verified ×3; test-only, zero product LOC. Migrations 058–067 now have a refactor-proof behavior net (GAP-14). |
