...
| P6 Commercialization | 068–081 | NOT STARTED | — | Order hardware during P2. **079 DONE out-of-order (2026-07-06, lexa-hub `task/079-dst-tou`, unmerged)** — ran early per its own scheduling note (gated only on 036, honoring P1 GAP-05 priority): DST/leap-smear TOU boundary tests + one inline `OptimalChargeWindow` fix (spring-forward gap hour-collapse undercounted window cost; no production caller today, zero bench impact); SOM-zone-matches-tariff-zone deployment requirement documented in lexa-hub CLAUDE.md; one KNOWN-GAP backlogged (25-hour fall-back day repeated-hour window-start limitation). **080 DONE out-of-order (2026-07-06, csip-tls-test `task/080-curve-adr`, docs-only, no hard prerequisite — Track F)** — AD-010 resolved: CSIP volt-var/volt-watt/ride-through curve dispatch **de-scoped** for V1.0 (survey: `docs/refactor/adr-inputs/curve-functions-survey.md`; today's-behavior trace confirms curve-linked `DERControlBase` fields are fetched, resolved, and displayed on the dashboard but silently dropped before reaching the scheduler — no crash/zero-value, scalar siblings unaffected); conformance-statement language drafted for TASK-081's regeneration; 09 checklist line closed; 10_BACKLOG carries the full ten-hop implementation bill (2S/5M/3L) + an S-effort ignored-curve-field alarm recommendation; market/certification questions asked, recorded unanswered with a revisit trigger (pilot/LOI signing or cert-lab scoping). No product code changed. |
