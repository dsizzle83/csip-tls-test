...
| P2 Device Reconciler (R1) | 025–033 | IN PROGRESS | — | Critical path. 025–031 DONE + MERGED: AD-013 schema + preservation ledger (025); reconciler core `internal/reconcile` 99.4% cov (026); battery→solar→EVSE all `reconciler-active` on the bench (027/028/029/030), campaign 33P/18D/0F/0B, Mode B scenarios (release-while-rebooting, curtailment-release) now PASS, interlock kept SENIOR, no INV-HUNT; CannotComply chain 5→3 hops via named `breachEpisodes` component merging optimizer breaches + reconciler non-convergence (031, gate 32P/19D/0F/0B). **032 DONE (2026-07-06, not yet merged):** legacy convergence machinery DELETED in 4 per-mechanism revertible commits (cmdDeduper+watchdog+breach-reset, legacy `lexa/control/*`+`evse/command` paths, `retryDevice.lastCtrl`/`reassertLocked`, redundant `restoreOnGenLimitClear`); reconciler is now the sole command owner (config `active` mandatory, off/shadow fatal); ledger L1–L4+L7 flipped to `deleted`, each gate PASS/accepted-DEGRADED (`docs/qa-task032/`), full campaign 35P/16D/0F/0B (≥ baseline), 10-cycle soak Principal-gated. Remaining: 033 (sign-off). |
| P3 Time & persistence | 034–043 | IN PROGRESS | — | 034 utilitytime core + 035 walker/scheduler (guards verbatim, failclosed untouched) + 036 hub/api/optimizer + 037 local clock-step (monotonic anchoring) all DONE + MERGED — **W4 closed, one time owner**. 039 journal library DONE + MERGED (zero consumers). Remaining: 038 (local-clock scenario), 040/041 (journal + snapshot wiring, need 031's breach component — now merged), 042/043 (retained-trust + power-cut scenarios). |
| P4 Observability & QA depth | 044–055 | IN PROGRESS | — | Parallel track. DONE + MERGED: 044 metrics, 045 slog+heartbeat, 047 httpwire+fuzz, 048 XML/bus fuzz, 053 int16/scale sweep (65M execs 0 crashers, GAP-07), 055 NaN-string bus robustness. CODE COMPLETE + MERGED (bench validation batched): 049/050/051 (dup-client-ID, disk-full, mqtt-storm), 052 (netem chaos harness). CODE COMPLETE, NOT YET MERGED (bench validation batched): 054 (dither sweeps — `task/054-dither`, awaiting review). Remaining: 046 (async publishes). |
