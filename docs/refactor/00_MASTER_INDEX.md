...
| P1 Shared modules (R2) | 019–024, 082 | IN PROGRESS (2026-07-05) | 021 lockstep: FAST 34P/17D/0F/0B; conformance 50/50; CSIP layers 1-3 3/3 (024: no fresh campaign run — CI/docs-only change, no bench access; see TASK-024 header) | 019, 020, 021, 022, 023, **024 DONE** (details in task headers + `docs/refactor/notes/` disposition docs). One shared codec proven on hardware; bench sunspec fork deleted; AD-003(e) interim vendoring (require+replace+vendor/) keeps hosted CI green with no lexa-proto host. **024**: TASK-004's raw-diff `lockstep` job retired; both repos' CI now run `proto-pin` (`scripts/check-proto-pin.sh`) comparing committed `proto.pin` SHAs (both seeded at lexa-proto `77e32e4`); `go.work` deleted from version control in both repos (gitignored, local-dev only); MTR-4 lockstep prose in both CLAUDE.md + BENCH.md replaced by the CI-gate pointer; AD-003 open question resolved, hosted-flip checklist recorded (AD-003(f), 10_BACKLOG.md). Open (human, same class as `LEXA_HUB_RO_TOKEN`): new `CSIP_TLS_TEST_RO_TOKEN` PAT for lexa-hub's `proto-pin` job. Remaining: 082 (bench derbase/device fork disposal + csip discovery/scheduler fork AD). |
