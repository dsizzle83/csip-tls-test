...
type DERSiteStatus struct {
	...
	// InverterStatus is the inverter operating state using the
	// csipmodel.Inverter* codes (0 = idle, 1 = operating, 2 = off,
	// 3 = fault) — the source for the 2030.5 DERStatus `inverterStatus`
	// element, which the CSIP CTP BASIC-028 step-4 element list makes
	// REQUIRED alongside genConnectStatus / operationalModeStatus /
	// readingTime.
	//
	// ADDITIVE (AD-006), and deliberately a plain uint8 rather than a
	// pointer, matching GenConnectStatus / OperationalMode above: its zero
	// value is csipmodel.InverterIdle, which IS the conservative floor a
	// producer with no fresh data should assert (the same floor those two
	// use). A producer that predates this field therefore decodes to that
	// conservative value rather than to an absence the consumer would have to
	// fabricate around — which matters because a REQUIRED element must not be
	// conditionally omitted from the PUT.
	InverterStatus uint8 `json:"inverter_status"`
