...
func SupportedV(topic string) int {
	...
	case strings.HasPrefix(topic, "lexa/der/") && strings.HasSuffix(topic, "/report"):
		// Per-device DER report family (D3.1/D3.3): DERDeviceReportTopic(device)
		// = "lexa/der/{device}/report". Distinct from TopicHubDERSite
		// ("lexa/hub/dersite") above — different prefix, no overlap.
		return DERDeviceReportV
...
// DERDeviceReportTopic returns the retained per-device DER report topic
// (product decision D3.1/D3.3): "lexa/der/{device}/report". Carries a
// DERDeviceReport (derdevice.go) — the per-device counterpart of the site
// aggregate on TopicHubDERSite. RETAINED at QoS 1 (PubQoS's non-measurement
// default): state, latest wins, a restarting consumer re-seeds from the
// broker. Writer: the gateway's per-device report producer; consumer:
// lexa-northbound's derreport manager.
func DERDeviceReportTopic(device string) string {
	return fmt.Sprintf("lexa/der/%s/report", device)
}

// SubDERDeviceReport matches every per-device DER report across all admitted
// devices — the subscribe filter the consumer (and the mosquitto ACL) uses.
const SubDERDeviceReport = "lexa/der/+/report"

// DeviceFromDERDeviceReportTopic extracts the device name from
// "lexa/der/{device}/report" (segment 2: lexa=0, der=1, {device}=2,
// report=3).
func DeviceFromDERDeviceReportTopic(topic string) string {
...
// "lexa/reconcile/{class}/{device}/report". Segments are 0-indexed over the
// FULL topic: lexa=0, reconcile=1, {class}=2, {device}=3, report=4 — so the
// class is segment 2 (the earlier index-1 was off by one, omitting the "lexa"
// segment, and returned "reconcile").
...
// "lexa/reconcile/{class}/{device}/report" — segment 3 (see the indexing note
// on ClassFromReconcileReportTopic; the earlier index-2 returned the CLASS,
// which silently no-op'd ApplyReconcileReport's device lookup — the reason the
// 704 control-setpoint readback echo never reached the register image).
