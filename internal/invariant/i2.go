...
func (i *i2) Check(ctx context.Context, w *World) (Result, error) {
	...
	// EVERY offending device, not just the first. Two DERs that both sat out the
	// convergence deadline are two devices to go and fix, and an identity that
	// named only the first reported one of them (IW15-032).
	convV, convChecked, convFacts, convReason, convDevs := i.convergence(w, obs)
	res.Checked += convChecked
	res.Verdict = Worse(res.Verdict, convV)
	res.Facts = append(res.Facts, convFacts...)
	if convV != Pass {
		for _, dev := range convDevs {
			key.note(convV, "failsafe-not-reached:%s", dev)
		}
	}

	relV, relChecked, relFacts, relReason, relDevs := i.release(w, obs)
	res.Checked += relChecked
	res.Verdict = Worse(res.Verdict, relV)
	res.Facts = append(res.Facts, relFacts...)
	if relV != Pass {
		for _, dev := range relDevs {
			key.note(relV, "failsafe-latched:%s", dev)
		}
	}
...
// is EVERY DEVICE the finding is about — the identity half of [Result.Keys],
// one identity per device, because a fleet in which three DERs never converged
// has three defects and the run must be able to count them.
func (i *i2) convergence(w *World, obs *Observation) (Verdict, int, []Fact, string, []string) {
	...
	if !okFS || !okDL {
		return Pass, 0, nil, "convergence arm needs -param failsafe_wmaxlimpct and -param failsafe_deadline " +
			"(the DUT publishes neither), so it was not asserted", nil
	}
	active := obs.Faults.ActiveOf(obs.At, ClassAuthorityLoss)
	if len(active) == 0 {
		return Pass, 0, nil, "no authority-loss fault is in force, so convergence is not under test right now", nil
	}
	...
	if elapsed < deadline {
		return Pass, 0, nil, fmt.Sprintf("still inside the stated %s convergence deadline (%s elapsed)", deadline, dur(elapsed)), nil
	}
	...
	for _, v := range derViews(obs) {
		...
		verdict = Fail
		devices = append(devices, v.Label)
		facts = append(facts,
	...
	if checked == 0 {
		return Pass, 0, nil, "no DER published a readable WMaxLimPct to compare against the failsafe", nil
	}
	return verdict, checked, facts, reason, devices
}
...
// promised how fast. Its last return value is EVERY DEVICE the finding is about
// — the identity half of [Result.Keys]; see convergence for why it is a list.
func (i *i2) release(w *World, obs *Observation) (Verdict, int, []Fact, string, []string) {
	...
	if !ok {
		return Pass, 0, nil, "no authority-loss fault has been cleared yet, so release is not under test", nil
	}
	if obs.At.Before(restored) {
		return Pass, 0, nil, "authority was restored after this observation", nil
	}
	if !obs.HeadEnd.Reachable {
		return Pass, 0, nil, "the head-end is unreachable, so there is no evidence authority actually returned", nil
	}
	...
	if !wantOK {
		return Pass, 0, nil, "the head-end serves no active control since authority returned, so the device is " +
			"correct to keep doing what it was doing", nil
	}
	...
	if checked == 0 {
		return Pass, 0, nil, "no DER published a readable WMaxLimPct to judge release by", nil
	}
	if pendingReason != "" {
		return Pending, checked, facts, pendingReason, devices
	}
	return Pass, checked, nil, "", nil
}
