...
// observe.go holds the parts of the live phase that are about IDENTITY and
// INTROSPECTION: which endpoint a check claims, what was done to the bench, and
// what the DUT's own journal says.
//
// The parts that are about TIMING moved to deterministic.go, and the reason is
// the whole point of LAB29-011. This file used to carry `watch` (sleep n poll
// intervals) and `awaitJournalEvidence` (watch the DUT's own journal for a line
// that looks like a reaction), and between them they decided when every row in
// this suite stopped looking. A sleep cannot tell "the client did not do it"
// from "the client had not got to it yet", which are opposite verdicts; and a
// journal watch lets the device under test choose its own examination window.
// Both are gone. Every row now arms at a named epoch, waits on the simulator's
// own barrier, and grades the simulator's own transaction ledger.
//
...
// no socket of its own to claim, which is why claimServer takes the weaker
// endpoint claim and why the citation phase re-derives that claim's soundness
// from the capture rather than assuming it.
//
// # Injection policy
//
...
//   - cleared, or the whole device restored to its baseline, in a defer that
//     survives cancellation — so a check that times out does not leave the
//     bench in a fault state for the next test case.
//
...
// The operator-supplied knobs.
//
// modbus-client.poll-interval-s is DELIBERATELY ABSENT. It used to tell this
// suite how long a poll cycle was so it could sleep that long; nothing here
// sleeps for a poll interval any more, so a number in that parameter would be
// a number nothing reads — worse than an error, because an operator who tuned
// it would believe they had changed something.
const (
...
func newObserver(rc *certify.RunCtx) (*observer, string) {
	...
	o := &observer{rc: rc, server: srv, injectOK: true}
